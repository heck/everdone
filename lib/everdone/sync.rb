...
module Everdone
    class Sync
        def initialize(config, todoist)
            ...
            @data = {}
            @current_item_id = nil
            items = @todoist.getItemsByProjectName(@config.todoist_sync_tracking_project)
        ...
        def close(new_processed)
            ...
            new_item_id = @todoist.addItemToProjectByName(@config.todoist_sync_tracking_project, 
